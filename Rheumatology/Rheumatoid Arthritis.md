Assessment:
- Always characterize morning stiffness
- Symmetric joint swelling (MCP, PIP, wrists)
- Look for ulnar deviation, swan-neck, and boutonniere deformity
- Check c-spine instability
- Check Hep B and TB history before starting biologics

Plan:
- Anti-CCP more specific than rheumatoid factor
- ANA, CRP, ESR
- UA with micro
- RF (-) and anti-CCP (-)? -> Seronegative RF still possible

Treatment:
- Methotrexate 10-25 mg/wk PO or SQ
	- Folic acid 1 mg daily to reduce ^^ toxicity
- Hydroxychloroquine and/or sulfasalazine
- Bridge with prednisone taper or intra-articular steroids

- Second line:
	- Add TNF-inhibitor (etanercept, adalimumab, infliximab)
	- Add non-TNF biologic (abatacept, tocilizumab, rituximab)
	- Add JAK inhibitor (tofacitinib, upadacitinib) for PO option

- Monitor with CBC/LFT q4-8 weeks on MTX, yearly once stable
- Target is DAS28 < 2.6, which is remission or low disease activity